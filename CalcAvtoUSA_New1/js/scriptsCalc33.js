  ...
  //  document.getElementById('res_remont').value=0;
    ...
    //var typeauction = document.getElementById('type_auction').value;
        ...
        if (typeauction=='iaai'){
            ...
            var url = "https://aleksandr.tupichenkov.com/CalcAvtoUSA_New1/js/sbor_iaai.xlsx";
            var oReq = new XMLHttpRequest();
            oReq.open("GET", url, true);
            oReq.responseType = "arraybuffer";
            oReq.onload = function(e) {
                ...
                var arr2 = new Array();
                for (var i = 0; i != data.length; ++i) {
                    arr2[i] = String.fromCharCode(data[i]);
                }
                var bstr2 = arr2.join("");
                var cfb = XLSX.read(bstr2, { type: 'binary' });

                var workbook = XLSX.read(bstr2, {type:"binary"});
