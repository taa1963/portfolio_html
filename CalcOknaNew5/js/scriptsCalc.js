...
$(document).ready(function() {
  ...
  // for (var i = 0; i < n; i++){
  //   console.log(n);
  //   var neww=width*0.307;
  //   console.log(neww);
  //   var block1=sliderLine[i];
  //   console.log(block1);
  //  // $(block1im[i]
  // // $(sliderLine[i]).style.width =neww+'px';
...
function okantf(element)
{
  ...
  var block1=$(element).parents('.slide-wrap');
  var block1im=$(block1).find('.cal_img1_block');
  var N=$(block1).find('.cal_img1_block').length;
  for (i=0; i<N; i++){
    $(block1im[i]).removeClass('cal_img1_block_active');
  }
  var block=$(element).parents('.cal_img1_block');
  $(block).addClass('cal_img1_block_active');
  if (NameKant==''){
...
function krepegf(element)
{
  ...
  var block1=$(element).parents('.calc_imgs');
  var block1im=$(block1).find('.cal_img1_block');
  var N=$(block1).find('.cal_img1_block').length;
  for (i=0; i<N; i++){
    $(block1im[i]).removeClass('cal_img1_block_active');
...
$('#but_block5_6').on("click",function(){
  ...
  var Zakasmailv=''
   ...
   Zakasmailv=Zakasmailv+'общая площадь окон:'+' '+sOkon+ 'кв.м.'+';'+"\n" ;

   Zakasmailv=Zakasmailv+'Стоимость:'+' '+PriseAll+ 'руб'+';'+"\n" ; 
   $('#zakas').text(Zakasmailv);

   $('#kantvv').text(NameKant);
   $('#typeMaterialvv').text(Namematerial);
   $('#typeKrepegvv').text(NameKrepeg);
   $('#typeOsnovavv').text(NameOsnova);
   $('#sOkonvv').text(sOkon);
   $('#Prisevv').text(PriseAll);
